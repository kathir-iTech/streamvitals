    ...
    try {
      ...
      setTimeout(() => {
        ...
        const interval = setInterval(() => {
          current += Math.max(1, Math.floor((target - current) * 0.08));
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        ...
        <div className="max-w-3xl w-full relative z-10">
          <div className="text-center mb-6">
            <h1 className="text-4xl font-black tracking-tight">Assessment Result</h1>
            ...
            <div className="relative text-center">
              <div className="relative w-40 h-40 mx-auto mb-6">
                <svg className="w-full h-full animate-spin-slow" style={{ animationDuration: '25s' }} viewBox="0 0 100 100">
                  ...
                  <circle cx="50" cy="50" r="45" fill="none" stroke={config.text === 'text-emerald-200' ? '#34d399' : config.text === 'text-amber-200' ? '#fbbf24' : '#f87171'} strokeWidth="2" strokeLinecap="round" strokeDasharray={`${score * 2.83} 283`} style={{ transform: 'rotate(-90deg)', transformOrigin: '50% 50%', transition: 'stroke-dasharray 2s ease-out' }} />
                </svg>
              ...
              <span className={`inline-block px-6 py-2 rounded-full border ${config.border} ${config.bg} ${config.text} text-xl font-black mb-3`}>{config.label}</span>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                { label: 'Data Status', value: dataStatus, style: dataStatus === 'SUFFICIENT' ? 'text-emerald-200' : dataStatus === 'PARTIAL' ? 'text-amber-200' : 'text-red-200', icon: Activity },
                { label: 'Evidence', value: `${assessed} assessed`, style: 'text-teal-200', icon: Shield },
                { label: 'Rules', value: `${drivers.length} applied`, style: 'text-teal-200', icon: Search },
              ].map((card, i) => (
          ...
          <div className="flex gap-3 animate-scaleIn" style={{ animationDelay: '0.8s' }}>
            <button onClick={() => window.location.href = '/confirm'} className="flex-1 py-4 bg-white/10 backdrop-blur-lg border border-white/20 text-white rounded-2xl font-bold hover:bg-white/20 transition-all focus:ring-2 focus:ring-teal-400 focus:outline-none">Retake Assessment</button>
            <Link href="/report" className="flex-1 py-4 bg-gradient-to-r from-teal-500 to-emerald-600 text-white rounded-2xl font-bold hover:opacity-90 transition-all focus:ring-2 focus:ring-teal-400 focus:outline-none shadow-lg shadow-teal-500/20 text-center">Generate Report →</Link>
          </div>
        </div>
